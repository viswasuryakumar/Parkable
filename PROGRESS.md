...
| C14 | `com.parkable.cli.GovDataImportCli` — composition root wiring `GovCityConfig` (per-city `GovDataFeed`+`GovRuleMapper`) to `RuleRepository.save`; direct construction now that X6-X9 exist (no reflection needed — that was only a contingency for writing this before Codex landed) | BLOCKED | 2026-07-21 | Blocked on X10 (see Codex table) — `GovRuleMapper.map` currently returns bare `Rule`s with no location, so there's nothing to bind an `ExtractionRecord.GpsCoordinates` to per rule. Also blocked-in-spirit on the separate parser_version filter bug (Requests & Blockers) — C14 can still write rows without it, but they won't be retrievable via `/check`/`/nearby` until that's fixed too. |
